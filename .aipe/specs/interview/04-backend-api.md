...
The previous version of this app synced to Notion. That whole layer was deleted in commit `dc8483a` once Supabase was stable — about 2,200 lines of mappers / rate-limiter / outbox-queue code that just went away. The migration was a 7-milestone plan in [`docs/buffr-cloud-sync-plan.md`](../../../docs/buffr-cloud-sync-plan.md); the design doc is [`docs/buffr-cloud-sync-spec.md`](../../../docs/buffr-cloud-sync-spec.md). The patterns I'm using — incremental pull by `updated_at`, soft delete via `deleted_at`, server-clock-anchored cursors via a `get_server_time()` RPC, last-write-wins conflict resolution — are textbook distributed-systems primitives applied at small scale.
...
If this app shipped to a wider audience I'd reconsider. For solo-dev daily use, the friction cost of explicit-only outweighs the cleanup cost of the occasional `#loop` → `#buffr` typo. The deviation from the original "explicit-only" design is documented inline above the auto-create branch so future-me knows it was intentional.
