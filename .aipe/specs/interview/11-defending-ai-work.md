...
- The original drops spec proposed a Next.js / Netlify stack because that was the AI's default from the project template. I rewrote the entire plan against React Native + Expo + SQLite before any code was written. That's documented in the repo's history — [`docs/spec.md`](../../../docs/spec.md) is the rewrite.
- The implementation plan for thinking-modes recommended flattening both the dashboard and `/todos` to chronological order. I pushed back and kept the dashboard ranked while flattening only `/todos`, with reasoning written down in [`docs/concepts.md`](../../../docs/concepts.md) so the call survives me.
...
- **Prose-canonical drops with two-pass matching** is a non-obvious architectural choice. No template would do that; in fact most production templates do the opposite (structured fields). The decision is documented in [`docs/spec.md`](../../../docs/spec.md) and the reasoning is in [`docs/concepts.md`](../../../docs/concepts.md).
