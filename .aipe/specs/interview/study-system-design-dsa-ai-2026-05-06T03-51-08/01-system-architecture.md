...
└───────────────────────────────────────────────────────┘
        │
        ▼
   SQLite (loopd.db, WAL mode) ← canonical
        │
...
### `[senior]` — "How does loopd guarantee that an entry's `todos_json` and the `todo_meta` table never drift?"
...
### `[arch]` — "If loopd had to support 100K users with shared multi-device sync, what changes?"
...
## The hard question — "loopd has no test suite. Why should I trust this code in production?"
