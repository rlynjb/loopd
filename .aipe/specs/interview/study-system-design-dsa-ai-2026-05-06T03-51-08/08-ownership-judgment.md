...
**Contrast.** A typical SaaS would ship auth in week one because *the product* is multi-user. loopd is *not* multi-user — it's a private journaling app where the cloud is a personal backup. The constraint that distinguishes: whether other users' data is in the same logical store. SaaS yes; loopd no, today.
