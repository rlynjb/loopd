...
Netflix's Hystrix library wraps every external call with two layers of protection. The first layer is retry with exponential backoff: when a downstream service blips for 200ms, the wrapper retries the call 2–3 times with growing delays, and the user never sees the failure. The second layer is the circuit breaker: when failures accumulate past a threshold (say 50% over the last 10 calls), the breaker OPENS and subsequent calls fail instantly without contacting the downstream — until a cool-down passes and one probe call tests recovery. AWS Lambda's concurrency limits ship a related pattern; the `react-query` `retry` config plus a global `onError` boundary builds the simpler half on the client; Cloudflare Workers expose circuit-breaker semantics via the platform itself. The two layers compose: retry handles transient failures, breaker stops piling work onto a dying service.
