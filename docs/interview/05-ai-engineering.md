...
> **The high-leverage chapter for AI-engineering interviews.** loopd has three LLM integrations across three cost tiers, and the architectural decisions around them are what separates this from "I called the OpenAI API."

Most projects with AI in them call one model for one job. loopd has three, and the choice of *which model for which job* is a deliberate product decision, not a default. The summary feature on the vlog editor uses primary-tier reasoning (Sonnet 4.6 or GPT-4o). The thinking-mode classifier on every captured todo uses cheap-tier reasoning (Haiku 4.5 or GPT-4o-mini). The expansion modal on non-todo rows uses primary-tier again. Different jobs, different costs, different latency budgets, different failure modes.
...
**Vlog summary** at [`summarize.ts:7-8`](../../src/services/ai/summarize.ts#L7-L8) is the same primary tier as expansion. Day-summary is reasoning-heavy and quality matters because the output drives the vlog editor's auto-compose.
