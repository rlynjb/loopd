...
What's missing and I'd prioritize: unit tests for [`scanTodos.ts`](../../src/services/todos/scanTodos.ts) and [`scanNutrition.ts`](../../src/services/nutrition/scanNutrition.ts) — they're pure functions with clear input/output contracts, perfect for testing. Snapshot tests for the merge logic in [`pullTodos`](../../src/services/notion/sync.ts) — the field-level merge rules are exactly the kind of code that breaks subtly when refactored. A fixture-based eval for the heuristic classifier — accuracy on a real corpus of `[]` lines.
...
[`docs/spec.md`](../spec.md) is the living architectural reference — every major decision is documented there with rationale. Major features get a separate plan document. The thinking-modes feature shipped via a 4-phase plan documented in [`docs/loopd-thinking-modes-spec.md`](../loopd-thinking-modes-spec.md). Each phase was independently shippable and each had its own scope estimate, success criteria, and explicit non-goals.
