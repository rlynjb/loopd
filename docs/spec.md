...
2. At commit time the prose is scanned: marked lines flow into typed records (todos in `todos_json` + `todo_meta`, nutrition in its own table, mentions in `thread_mentions`) without leaving the prose itself.
...
6. Optional: Notion syncs entries + todos (and, opt-in, habits cadence + threads) bidirectionally.
...
| `#tag` | inline anywhere in entry prose OR in a `[]` todo line; case-insensitive; ignored inside backticks/code blocks | `thread_mentions` junction (one row per occurrence, deduped per-line per-slug) | `thread_id`, `tag_text` (literal-as-typed), source pointer (`entry_id` and/or `todo_id`), `source_line` |
...
2. **Pass 2 — line-index fallback** via a `sourceLine` field on the record. Handles in-place edits — changing `[] call mom` to `[] call dad` keeps the same row, preserves `id` / `done` / `createdAt`. For threads, fallback also matches `(thread_id, tag_text)` within ±3 lines.

For `#tag` specifically: unknown slugs **auto-create a thread on save** (deviation from a stricter "explicit-only" model — trades typo-safety for ergonomics; the inline `+ create` chip still works for immediate feedback in the editor). Manual reorder isn't applicable since mentions are line-anchored.
...
Nutrition / Habits / Threads CRUD pages live under `/more/{nutrition,habits,threads}`. The dashboard's **DAILY SCHEDULE** section (§4 Home) and the per-thread detail page at `/threads/[id]` give in-context entry points so the More tab is the management hub, not the daily-use surface.
...
- **Backfill migrations**, all SecureStore-gated:
  ...
  - `thread_mentions_backfill_v1_done` — lazy backfill that scans every entry + todo for `#tag` matches against the threads table; only runs after the user has at least one thread (otherwise no-op until the next boot)
...
- **DAILY SCHEDULE** (§6.10) — combined HABITS + THREADS section. Both rendered as habit-row-style strips (80px name + 14-cell flex strip + 36px right-side count/nav). Bucketed by `time_of_day` (morning → midday → evening → anytime); adaptive mini-headers appear once 2+ buckets are populated by either type. `manage →` link routes to `/more`.
...
- **Commit-time scanners** run from [useEntries.editEntry](../src/hooks/useEntries.ts) when text changes:
  ...
  - `scanThreadsForEntry` (fire-and-forget; runs **after** scanTodos because it needs the final todo IDs for `[]`-line tag attribution) reconciles `#tag` mentions in entry prose AND inside each todo's text against `thread_mentions`. Unknown slugs auto-create a thread.
...
- **Tag autocomplete** — when the cursor sits immediately after a `#xyz` partial token, [TagAutocomplete](../src/components/journal/TagAutocomplete.tsx) renders a sibling chip bar (above the keyboard, same Z order as nutrition autocomplete). Recency-sorted existing threads + a `+ create #xyz` affordance for unknown slugs. Same component is reused on `/todos` for both new-todo and edit-todo inputs.
...
- **Threads filter** (`Threads:` label + horizontal scroll, only renders if at least one thread exists): ALL + one chip per thread with `#slug` and a count of todos tagged. Filter ANDs with the other two.
...
List of links with one-line stats: nutrition (entries this week), habits (active count + due-today count), threads (active count + going-stale count). Settings + Notion Sync links live in the global header (sync icon + gear icon at top of every screen via `HomeHeader`).
...
Trash icon on each row hard-deletes (with confirm); past entries' `habits_json` references are preserved.

### `app/more/threads.tsx` — threads CRUD
List of all threads (active + archived tabs). Editor sheet exposes name, slug (auto-derived from name unless user has manually edited), time-of-day, and target cadence (days). Pin / archive / hard-delete actions per row.

### `app/threads/[id].tsx` — thread detail
Per-thread aggregate view. Header shows name + colored dot + pin star + staleness label + stats. Three sections:
- **OPEN** — open todos tagged with this thread, newest first. Tap row → `/todos`. Tap checkbox → toggle done.
- **DONE** — recent 5 completed todos (with `(recent 5 of N)` counter when truncated).
- **ENTRIES** — every prose mention with line excerpt (~140 chars), sorted newest first. Tap → opens that day's journal.

Reachable by tapping a thread row's `→` arrow on the dashboard tracker, or from the threads CRUD via long-press / explicit "view" affordance.
...
|---|---|---|---|
| `habits` | `id` | `label`, `sort_order`, `slug`, `icon`, `color`, `cadence_type`, `cadence_days` (JSON), `cadence_count`, `archived` (deprecated for habits — kept on row, not surfaced in UI), `time_of_day`, `notion_page_id`, `notion_last_synced`, `updated_at` | User's repeatable daily disciplines + cadence + time-of-day bucket |
...
- **`Habit`** — `{ id, label, sortOrder, slug?, icon?, color?, cadenceType?, cadenceDays?, cadenceCount?, timeOfDay?, notionPageId?, notionLastSynced?, updatedAt? }`. New fields are optional on the type so existing call sites that construct minimal habits (e.g. the Notion-options sync path) keep compiling; the DB layer fills sensible defaults (`cadence_type='daily'`, `time_of_day='anytime'`).
- **`CadenceType`** — `'daily' | 'weekdays' | 'weekly' | 'specific_days' | 'n_per_week'`.
- **`TimeOfDay`** — `'morning' | 'midday' | 'evening' | 'anytime'`. Sort order on the dashboard tracker: morning → midday → evening → anytime. `'anytime'` is the catch-all default.
...
- **`ThreadCard`** — computed view shape consumed by the dashboard. `{ thread, lastMentionAt, daysSinceLast, staleness, entriesThisWeek, openTodos, recentTodos, activeDates }`. `activeDates` is a `Set<string>` of YYYY-MM-DD where the thread had a manual-touch mention in the last 14 days (drives the dashboard 14-cell strip; `#tag` mentions in prose are deliberately excluded).
...
Globally-defined habits with cadence rules + time-of-day buckets. Per-entry logging stays on `entries.habits_json` (no schema break).
...
**CRUD** at `/more/habits`. Sheet editor exposes name, time-of-day chips, cadence-type radio, day-picker (for weekly/specific_days), count picker (for n_per_week). Trash icon hard-deletes; past entries' `habits_json` references stay (just dangle).
...
**Auto-create on save.** Unknown slugs **auto-create** a Thread row at scan time using the literal-as-typed `tagText` as the display name and the lowercased text as the slug. This is a deliberate ergonomic deviation from the original "explicit-only" idea (the inline `+ create` chip on the autocomplete still works and is the immediate-feedback path).
...
**Manual touch.** From the dashboard tracker, tapping a thread row writes a `thread_mentions` row with `entry_id IS NULL AND todo_id IS NULL` (see [touch.ts](../src/services/threads/touch.ts)). This deviates from Principle 11 (mentions are derived from prose) and is the **only** documented exception. Justified because (a) the schema permits it, (b) the 14-day strip + staleness math compose uniformly across all mention shapes, (c) toggling off only deletes the manual row.
...
**Lazy backfill** ([threads/migrate.ts](../src/services/threads/migrate.ts)) — SecureStore-gated under `thread_mentions_backfill_v1_done`, but the gate ALSO checks "user has at least one thread" before running. Without that check, a fresh install would scan thousands of entries to find zero matches. The gate flips on the first boot AFTER a thread exists.

**Mentions are NOT synced to Notion** (see §6.10). They're derived from entries/todos which already sync.
...
Both row types share the same layout (80px name + flex 14-cell strip + 36px right-side number). Habit rows use the cadence-aware streak count; thread rows show a `→` nav arrow (tap → `/threads/[id]` detail page). Tapping the thread row body toggles a manual touch (§6.6).

The 14-cell strip on thread rows is driven exclusively by manual touches — `#tag` mentions in prose do NOT light up the strip. This was a deliberate decision: prose mentions show on the thread detail page (where mentions are the whole point), but they don't paint the dashboard's "done today" indicator.
...
- **`syncAllHabits()`** — Optional Habits DB (separate from the Entries-DB multi-select that governs identity). When configured via `HABITS_DB_ID`, this carries cadence + time-of-day + slug + icon/color metadata bidirectionally. Slug is **local-only** (Notion edits rejected on pull, log warning). Schema-gap tolerance via `detectMissingHabitProperties`. Silent no-op when DB ID is unset.
...
- Boot-time auto-sync chain (only if Notion is configured AND auto-sync is on): `syncAll → syncAllTodos → syncAllHabits → syncAllThreads`.
